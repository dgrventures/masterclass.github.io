...
[`../../../../projectapps/masterclass_fin/src/deliverables/`](../../../../projectapps/masterclass_fin/src/deliverables/). A run folder may hold
scratch files, but anything that ships belongs in `deliverables/`.

## Folder naming — one run = one commit

```
dev/projects/masterclass_fin/runs/<NN>_<TAG>_<slug>/      ↔  commit  YYMMDD-<TAG><NN>-<slug>
```
...
3. Copy the template into it:
   ```bash
   cp dev/projects/masterclass_fin/runs/_template/RUN.md dev/projects/masterclass_fin/runs/<NN>_<TAG>_<slug>/RUN.md
   ```
4. Fill in **Goal** and **Scope** before starting.
5. Do the work in `deliverables/`. If the case website changed, run the publish
   step (casus: `node …/22_casepage/assemble.mjs`; lecture: `cp … lezing.html`; see
