...
Lives under `dev/project/deliverables/22_casepage/src/`:
```
src/
...
- **Publish:** `node dev/project/deliverables/22_casepage/src/assemble.mjs`. The
...
## Lecture (JB)
`dev/project/deliverables/10_lezing/02_lezing.html` → `cp` to repo-root `lezing.html`.
Its non-app source materials (examples, validation xlsx) live in
`dev/project/data/projectdeliverables/10_lezing/`.

## Project data (`dev/project/data/`)
...
## Versions
Top-level `versions/version_<YYMMDD>_<label>/output/` — kept snapshots of the deployed
output (`index.html`, `lezing.html`). See [`/versions/README.md`](../../../versions/README.md).

## Repo map
```
index.html · lezing.html · specs/.gitkeep · versions/
dev/general/  (reusable: workflow·design·skills·architecture)
dev/project/  plan/(brief·context·backlog·architecture) · tracks/ · data/ · deliverables/
dev/runs/     run logs
```
